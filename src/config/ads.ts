export const ADS = {
  // HilltopAds MultiTag - Top Banner
  topBanner: `
    <script>
    (function(qnpk){
    var d = document, s = d.createElement('script'), l = d.scripts[d.scripts.length - 1];
    s.settings = qnpk || {};
    s.src = "\\/\\/selfassured-celebration.com\\/b.XjVVsGdDG\\/l\\/0\\/YNWFcT\\/neam-9quKZSUMlgkXP\\/TNYj4bNSDjcB1ANxjeUGt\\/NBjLgV0WNdzPUo2\\/OqQG";
    s.async = true;
    s.referrerPolicy = 'no-referrer-when-downgrade';
    s.onload = function() { console.log("✅ Top Banner: Loaded"); };
    s.onerror = function() { console.error("❌ Top Banner: Failed to load (Check AdBlock)"); };
    l.parentNode.insertBefore(s, l);
    })({})
    </script>
  `,

  // HilltopAds Popunder Script - Middle
  middleBanner: `
    <script>
    (function(msnibcb){
    var d = document, s = d.createElement('script'), l = d.scripts[d.scripts.length - 1];
    s.settings = msnibcb || {};
    s.src = "\\/\\/plasticdamage.com\\/c.D\\/9G6Ybk2c5FljSWWkQ\\/9nNmj-gh0pNoziUb4wOSSb0o2MOvDxQc3vN-TJkdzO";
    s.async = true;
    s.referrerPolicy = 'no-referrer-when-downgrade';
    s.onload = function() { console.log("✅ Popunder: Active"); };
    s.onerror = function() { console.error("❌ Popunder: Failed/Blocked"); };
    l.parentNode.insertBefore(s, l);
    })({})
    </script>
  `,

  // HilltopAds In-page Push - Sidebar 1
  sidebarAd1: `
    <script>
    (function(inf){
    var d = document, s = d.createElement('script'), l = d.scripts[d.scripts.length - 1];
    s.settings = inf || {};
    s.src = "\\/\\/selfassured-celebration.com\\/bIX.VrsQdQGAls0-YgWwcU\\/ieomv9huKZBUOlIk\\/PGTuYz4KNSD-cy2RMDjPU\\/tPNnjGgP0LN\\/zDY_y\\/ORQG";
    s.async = true;
    s.referrerPolicy = 'no-referrer-when-downgrade';
    s.onload = function() { console.log("✅ Sidebar 1: Loaded"); };
    s.onerror = function() { console.error("❌ Sidebar 1: Failed"); };
    l.parentNode.insertBefore(s, l);
    })({})
    </script>
  `,

  // HilltopAds MultiTag (Small) - Sidebar 2
  sidebarAd2: `
    <script>
    (function(hzvbcz){
    var d = document, s = d.createElement('script'), l = d.scripts[d.scripts.length - 1];
    s.settings = hzvbcz || {};
    s.src = "\\/\\/selfassured-celebration.com\\/bQX.VysGdLGZle0kYcWmcL\\/veCm\\/9\\/ubZmUglQkIP-TJY\\/4YNPD\\/cz2LNUDLEuttNZjEg\\/0uNhzIYR0aNEQ_";
    s.async = true;
    s.referrerPolicy = 'no-referrer-when-downgrade';
    s.onload = function() { console.log("✅ Sidebar 2: Loaded"); };
    s.onerror = function() { console.error("❌ Sidebar 2: Failed"); };
    l.parentNode.insertBefore(s, l);
    })({})
    </script>
  `,

  // HilltopAds Banner (Old Video Slider replaced) - Footer
  footerBanner: `
    <script>
    (function(hdw){
    var d = document, s = d.createElement('script'), l = d.scripts[d.scripts.length - 1];
    s.settings = hdw || {};
    s.src = "\\/\\/selfassured-celebration.com\\/b.XjVVsGdDG\\/l\\/0\\/YNWFcT\\/neam-9quKZSUMlgkXP"; 
    s.async = true;
    s.referrerPolicy = 'no-referrer-when-downgrade';
    s.onload = function() { console.log("✅ Footer Banner: Loaded (Clean)"); };
    s.onerror = function() { console.error("❌ Footer Banner: Failed (Replaced Video Slider)"); };
    l.parentNode.insertBefore(s, l);
    })({})
    </script>
  `
};
