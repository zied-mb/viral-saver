...
const Home: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${bg} ${text} overflow-x-hidden`}>
      ...
      {/* ── Footer ── */}
      <footer className={`border-t ${darkMode ? "border-white/5" : "border-slate-100"} py-8 sm:py-12 px-6`}>
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 sm:gap-8 mb-8 text-center md:text-left">
            <div className="flex items-center gap-2">
              <img src="/logo.png" alt="ViralSaver logo" className="w-6 h-6 sm:w-7 sm:h-7 object-contain" />
              <span className="font-black text-lg sm:text-xl tracking-tight bg-gradient-to-r from-violet-400 via-pink-400 to-cyan-400 bg-clip-text text-transparent">ViralSaver</span>
            </div>
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6 text-xs sm:text-sm">
              {["Privacy Policy", "Terms of Service", "Contact", "DMCA"].map((link) => (
                <a key={link} href="#" className={`transition-colors ${darkMode ? "text-white/30 hover:text-white/60" : "text-slate-400 hover:text-slate-600"}`}>{link}</a>
              ))}
            </div>
          </div>
          <div className={`border-t ${darkMode ? "border-white/5" : "border-slate-100"} pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-center`}>
            <p className={`text-[10px] sm:text-xs ${darkMode ? "text-white/20" : "text-slate-300"}`}>© {new Date().getFullYear()} ViralSaver. All rights reserved.</p>
            <p className={`text-[10px] sm:text-xs ${darkMode ? "text-white/15" : "text-slate-300"}`}>For personal use only. Respect creators' rights.</p>
          </div>
        </div>
      </footer>
    </div>
