import React, { useEffect, useRef } from "react";
import { ADS } from "@/config/ads";

interface AdsBannerProps {
  type: "top" | "middle" | "footer" | "sidebar-sm" | "result-inline" | "notification";
  className?: string;
}

const AD_DIMENSIONS = {
  top: { height: 90, width: "1200px" },
  middle: { height: 100, width: "300px" },
  footer: { height: 100, width: "300px" },
  "sidebar-sm": { height: 250, width: "300px" },
  "result-inline": { height: 250, width: "300px" },
  notification: { height: 0, width: "0px" },
};

const SCRIPT_SOURCES: Record<string, string> = {
  notification:
    "//selfassured-celebration.com/brXEV.sbdaGqlq0BYlWqcw/teEm/9/uXZzUIl_ksPzTqYW4UNBD/cf2uMFjiUmthNyjDgO0jN-zAYxyMOKQa",

  middle:
    "//selfassured-celebration.com/bHXLV/s.dJGHlS0HYXWBcl/wezmJ9vu/ZqU/lskaPJTgYq4cNATHQY0EOgTqc/tkNoj/gR1PNiD_U/wOMYQX",

  footer:
    "//selfassured-celebration.com/b.XeVQsXdDGcls0XYFWicl/oecm/9-u/Z/UgljktPfTWYE4LNXTsQr1YOgDRU/t/N/jYg/1xNsDnUI4aOoQh",

  "sidebar-sm":
    "//selfassured-celebration.com/bUXpVks.dcGblt0/Y/W/cM/CekmB9eucZnUQlwkuPsTyYz4qNETtIgyPMxj/ELtJN/jRg/1/M/jhI/ybNLQq",

  "result-inline":
    "//selfassured-celebration.com/bxXzVjs.dDGyla0EYtWVcn/xeAmA9NuzZJUulnkWPZT_Y/4bNgTRI/yrO/DtUdtSNrj/gK1lM/jGIp4/OGQE",
};

const AdsBanner: React.FC<AdsBannerProps> = ({ type, className = "" }) => {
  const dim = AD_DIMENSIONS[type];
  const adContainerRef = useRef<HTMLDivElement>(null);
  const loadedRef = useRef(false);

  const resolveValue = (val: any) =>
    val && typeof val === "string" && val !== "[object Object]" ? val : "";

  const getAdId = () => {
    switch (type) {
      case "top":
        return resolveValue(ADS.topBanner);
      case "sidebar-sm":
        return resolveValue(ADS.sidebarAd1);
      case "result-inline":
        return resolveValue(ADS.sidebarAd2);
      case "middle":
        return resolveValue(ADS.middleBanner);
      case "footer":
        return resolveValue(ADS.footerBanner);
      case "notification":
        return resolveValue(ADS.adNotification);
      default:
        return "";
    }
  };

  const adId = getAdId();

  useEffect(() => {
    const isValid =
      adId && typeof adId === "string" && adId.trim() !== "" && adId !== "[object Object]";

    if (!isValid) return;

    if (type === "notification") {
      const script = document.createElement("script");
      script.src = SCRIPT_SOURCES.notification;
      script.async = true;
      script.setAttribute("data-cfasync", "false");

      const timeout = setTimeout(() => {
        document.body.appendChild(script);
      }, 10000);

      return () => clearTimeout(timeout);
    }

    const container = adContainerRef.current;
    if (!container) return;

    const observer = new IntersectionObserver(
      (entries) => {
        const entry = entries[0];

        if (entry.isIntersecting && !loadedRef.current) {
          loadedRef.current = true;

          const script = document.createElement("script");
          script.src = SCRIPT_SOURCES[type];
          script.async = true;
          script.setAttribute("data-cfasync", "false");
          script.referrerPolicy = "no-referrer-when-downgrade";

          container.appendChild(script);
        }
      },
      { threshold: 0.2 }
    );

    observer.observe(container);

    return () => observer.disconnect();
  }, [adId, type]);

  if (type === "notification") return null;

  if (!adId && type === "top") {
    return (
      <div className={`w-full flex justify-center items-center my-6 px-4 overflow-hidden ${className}`}>
        <a
          href="https://mdbcollection.com"
          target="_blank"
          rel="noopener noreferrer"
          className="relative w-full group overflow-hidden rounded-xl border border-white/10 shadow-2xl transition-all duration-300 hover:border-white/20"
          style={{ maxWidth: dim.width }}
        >
          <div className="h-[90px] md:h-[130px] w-full transition-all duration-500">
            <img
              src="/mdb-banner.jpg"
              alt="MDB Collection"
              className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
            />
          </div>

          <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md text-[9px] font-bold text-white/80 px-2 py-0.5 rounded-full uppercase tracking-widest border border-white/5">
            Sponsored
          </div>
        </a>
      </div>
    );
  }

  if (!adId) return null;

  const responsiveClasses =
    type === "middle" || type === "footer" ? "md:hidden" : "";

  return (
    <div
      className={`ads-container w-full flex justify-center items-center my-4 ${responsiveClasses} ${className}`}
    >
      <div
        ref={adContainerRef}
        style={{
          width: "100%",
          maxWidth: dim.width,
          minHeight: `${dim.height}px`,
        }}
        className="flex justify-center items-center"
      />
    </div>
  );
};

export default AdsBanner;
